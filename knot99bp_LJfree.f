...
c	tr=0.125d0
	part=men
     ...
     &'thermo_',fstr,'_99bp_dt0.01_topo_endfree.dat'
	open(14,file=fname1,status='unknown')


        write(fname3,'(a,f5.3,a)')
     &'dishb_',fstr,'_99bp_dt0.01_topo_endfree.dat'
...
c        CALL GOSAMPLE
	open(20,file='trefoil_short_loose.txt',status='old')
...
c        write(3,*)tr
     ...
     &'traj_',fstr,'_',tr,'_99bp_dt0.005_topo_endfree.dat'
...
c            else if (dy2.lt.rcut) then
c              AA=1/dy2**12
c              BB=1/dy2**6
c              ENE=4.0d0*(AA-BB)
c              ENER=ENER+ENE
c              FCE=48.0d0*AA-24.0d0*BB
c              fy(i)=fy(i)-FCE
